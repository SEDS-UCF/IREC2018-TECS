...
bool BME280::begin(uint8_t addr/* = BME280_ADDRESS*/) {
	_i2caddr = addr;
	return init();
}
...
void BME280::setSampling(sensor_mode mode, sensor_sampling tempSampling, sensor_sampling pressSampling, sensor_sampling humSampling,
...
void BME280::write8(uint8_t reg, uint8_t value) {
	...
	bcm2835_i2c_write((const char*)&reg, 1);
	bcm2835_i2c_write((const char*)&value, 1);
}
...
uint16_t BME280::read16(uint8_t reg) {
	...
	bcm2835_i2c_setSlaveAddress(_i2caddr);
	bcm2835_i2c_write((const char*)&reg, 1);
	bcm2835_i2c_read((char*)&values, 2);
...
uint32_t BME280::read24(uint8_t reg) {
	...
	bcm2835_i2c_setSlaveAddress(_i2caddr);
	bcm2835_i2c_write((const char*)&reg, 1);
	bcm2835_i2c_read((char*)&values, 3);
	value = values[0];
	value <<= 8;
	value = values[0];
	value <<= 8;
...
float BME280::readHumidity(void) {
	...
	v_x1_u32r
		= (((((adc_H << 14) - (((int32_t)_bme280_calib.dig_H4) << 20) - (((int32_t)_bme280_calib.dig_H5) * v_x1_u32r)) + ((int32_t)16384)) >> 15)
...
void BME280::fillPT(float* pressure, float* temperature) {
...
*/
/**************************************************************************/
float BME280::readAltitude(float atmospheric/* = NAN*/, float seaLevel/* = 1013.25*/) {
	...
	if(atmospheric == NAN)
		atmospheric = readPressure() / 100.0F;
